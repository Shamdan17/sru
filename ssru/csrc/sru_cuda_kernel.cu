                        ...
                        const int is_custom)
{
    ...
    const auto* vp1 = is_custom ? (weight_c + col) : (weight_c + (col%d));
    //no reset gate
    ...
    const auto* xp = (skip_type == 0) ? NULL : ((skip_type == 1) ? (x + col) : (up + 2));
    ...
    for (int row = 0; row < len; ++row)
    {
        if ((pad_p == NULL) || !(*pad_p)) {
            ...
            *hp = val * mask;
            *hp = skip_type ? (val * mask  + x_val) : (val * mask);
        } 
        //else {
...
//     const auto* vp1 = is_custom ? (weight_c + col + (len-1)*ncols) : (weight_c + (col%d));
//     // const auto* vp2 = is_custom ? (weight_c + col*2 + 1 + (len-1)*ncols*2) : (weight_c + (col%d) + d);
//     auto* gvp1 = is_custom ? (grad_wc + col + (len-1)*ncols) : (grad_wc + col);
                        ...
                        const int is_custom)
{
    ...
    for (int row = len-1; row >= 0; --row)
    {
        ...
        //vp1 = is_custom ? (vp1 - ncols*2) : vp1;
        // vp2 = is_custom ? (vp2 - ncols*2) : vp2;
        gvp1 = is_custom ? (gvp1 - ncols) : gvp1;
        // gvp1 = is_custom ? (gvp1 - ncols*2) : gvp1;
        // gvp2 = is_custom ? (gvp2 - ncols*2) : gvp2;
    }
                        ...
                        const int is_custom)
{
    ...
    const auto* vp1 = is_custom ? (weight_c + col) : (weight_c + (col%d2));
                           ...
                           const int is_custom)
{
    ...
    const auto* vp1 = is_custom ? (weight_c + col) : (weight_c + (col%d2));
    // const auto* vp2 = is_custom ? (weight_c + col*2 + 1) : (weight_c + (col%d2) + d2);
    auto* gvp1 = is_custom ? (grad_wc + col) : (grad_wc + col);
    ...
    for (int cnt = 0; cnt < len; ++cnt)
    {
        ...
        vp1 = is_custom ? (vp1 - ncols_) : vp1;
        // vp2 = is_custom ? (vp2 - ncols_*2) : vp2;
        gvp1 = is_custom ? (gvp1 - ncols_) : gvp1;
