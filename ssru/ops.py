...
def elementwise_recurrence_gpu(U: Tensor,
    ...
    """
    from .cuda_functional import SSRU_Compute_GPU
...
def elementwise_recurrence_naive(U: Tensor,
    ...
    if not has_skip_term:
        ...
        # print("K == 3 code doesn't work properly!")
    else:
        print("k!=2 and skip term is true! Code doesn't work properly!")
        x_prime = U[..., 3]
    ...
    for di in range(bidir):
        ...
        for t in time_seq:
            ...
            # assert x_prime is None
            if x_prime is not None:
                h_t = xp[t] + g_c_t * mask_c_
                # print("Using skip connection not valid.")
                # exit(1)
            else:
