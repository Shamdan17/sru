...
class SSRU_Compute_GPU(Function):
    ...
    def forward(ctx, u, x, weight_c, bias,
        ...
        skip_type = 0 if not has_skip_term else (1 if k_ == 2 else 2)
    ...
    def backward(ctx, grad_h, grad_last):
        ...
        skip_type = 0 if not ctx.has_skip_term else (1 if k_ == 2 else 2)

        is_custom = len(weight_c.size()) > 1

        grad_u = u.new_zeros(*u.size())
        grad_init = x.new_zeros(batch, d * bidir)
        grad_x = x.new_zeros(*x.size()) if skip_type > 0 and k_ == 2 else None
        # No reset gate
        # grad_bias = x.new_zeros(2, batch, bidir * d)
        grad_bias = x.new_zeros(1, batch, bidir * d)
        if not is_custom:
            grad_wc = x.new_zeros(1, batch, bidir * d)
        else:
        ...
        backward_func(
            grad_u,
            grad_x if skip_type > 0 and k_ == 2 else empty_ftensor,
            grad_wc,
            grad_bias,
            grad_init,
            u.contiguous(),
