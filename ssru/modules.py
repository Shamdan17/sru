...
from ssru.ops import (elementwise_recurrence_cpu,
...
class SSRUCell(nn.Module):
    """
    A single SSRU layer as per `LSTMCell`, `GRUCell` in Pytorch.
    """
    ...
    def __init__(self,
        ...
        """Initialize the SSRUCell module.
        ...
        bidirectional: bool, optional
            if True, set the module as a bidirectional SSRU
            (default=False)
        n_proj: int, optional
        ...
        # number of sub-matrices used in SSRU
        self.num_matrices = 2
        ...
        # SSRU doesn't have a reset gate
        self.weight_c = nn.Parameter(torch.Tensor(self.output_size))
    ...
    def reset_parameters(self):
        """Properly initialize the weights of SSRU, following the same
        recipe as:
        Xavier init:
        ...
        if self.custom_m is None:
            ...
            if self.rescale and self.has_skip_term and self.num_matrices == 3:
                scale_val = (1 + math.exp(bias_val) * 2)**0.5
                w[:, :, :, 3].mul_(scale_val)
        else:
    ...
    def forward(self,
        ...
        """The forward method of the SSRU layer.
        """
        ...
        # apply layer norm before activation (i.e. before SSRU computation)
        residual = input
    ...
    def compute_UV(self,
        ...
        """
        SSRU performs grouped matrix multiplication to transform the
    ...
    def get_dropout_mask_(self,
                          ...
                          p: float) -> Tensor:
        """
        Composes the dropout mask for the `SSRUCell`.
        """
        b = self.bias.data
...
class SSRU(nn.Module):
    """
    Implementation of Simple Recurrent Unit (SSRU)
    """
    ...
    def __init__(self,
        ...
        hidden_size: int
            ...
            direction*
        num_layers: int
            the number of stacked SSRU layers (default=2)
        dropout: float, optional
            the dropout value applied between layers (default=0)
        rnn_dropout: float, optional
            [DEPRECATED] the variational dropout value (default=0)
        ...
        if rnn_dropout > 0:
            warnings.warn("rnn_dropout > 0 is deprecated and will be removed in"
                          "next major version of SSRU. Please use dropout instead.")
        if use_tanh:
            warnings.warn("use_tanh = True is deprecated and will be removed in"
                          "next major version of SSRU.")
    ...
    def forward(self, input: Tensor,
                c0: Optional[Tensor] = None,
                mask_pad: Optional[Tensor] = None) -> Tuple[Tensor, Tensor]:
        """The forward method of SSRU module
        ...
        # packing at all, but makes SSRU usage compatible with nn.RNN usage
        orig_input = input
    ...
    def make_backward_compatible(self):
        ...
        if hasattr(self, 'n_in'):
            if len(self.ln_lst):
                raise Exception("Layer norm is not backward compatible for ssru<=2.1.7")
            if self.use_weight_norm:
                raise Exception("Weight norm removed in ssru>=2.1.9")
            ...
            for cell in self.rnn_lst:
                ...
                if cell.activation_type > 1:
                    raise Exception("ReLU or SeLU activation removed in ssru>=2.1.9")
